#!/usr/bin/env python3
"""Train and evaluate the reproducible YOLO11n baseline v2."""
...
DATA = ROOT / "data/v2/yolo/dataset.yaml"
RUNS = ROOT / "runs/detect"
RESULTS = ROOT / "results/yolo11n_baseline_v2"
...
def class_instances(split: str) -> Counter:
    counts = Counter()
    for path in (ROOT / f"data/v2/yolo/labels/{split}").glob("*.txt"):
...
def metric_rows(split: str, metrics):
    ...
    by_class = {int(class_id): index for index, class_id in enumerate(box.ap_class_index)}
    rows = []
    for i, name in enumerate(CLASSES):
        index = by_class.get(i)
        if index is None:
            rows.append([split, i, name, "N/A", "N/A", "N/A", "N/A", instances[i]])
        else:
            rows.append([split, i, name, float(box.p[index]), float(box.r[index]), float(box.ap50[index]), float(box.maps[i]), instances[i]])
    return rows
...
def main() -> None:
    ...
    run_name = unique_name("yolo11n_baseline_v2")
    ...
    best_model.predict(
        source=str(ROOT / "data/v2/yolo/images/val"), imgsz=640,
        conf=0.25, device=0, save=True, project=str(RESULTS / "predictions"), name="val",
    )
    best_model.predict(
        source=str(ROOT / "data/v2/yolo/images/test"), imgsz=640,
